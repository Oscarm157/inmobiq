...
export function ValuationDetailClient({ result, narrative, property }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero — sin score grande (lo muestra ScoreSlider abajo) */}
      <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${cfg.heroBg} p-8 md:p-10 border border-slate-100 dark:border-slate-800/50`}>
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" className="relative">
          ...
          {/* Veredicto prominente + percentil */}
          <motion.div variants={fadeUp} className="flex items-center gap-3 mb-5">
            <span className={`inline-flex items-center px-4 py-2 rounded-xl text-base font-bold ${cfg.chip}`}>
              {cfg.label}
            </span>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              percentil {result.price_percentile} · {result.price_premium_pct > 0 ? "+" : ""}{result.price_premium_pct.toFixed(1)}% vs zona
            </span>
          </motion.div>

          {/* Property title */}
          <motion.h1 variants={fadeUp} className={`text-2xl md:text-3xl font-extrabold mb-4 leading-tight ${cfg.accent}`}>
            {TYPE_LABELS[property.property_type]} en {property.listing_type}
            <span className="text-slate-800 dark:text-slate-100"> · {result.zone_name}</span>
          </motion.h1>
