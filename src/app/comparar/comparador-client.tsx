...
const ZONE_COLORS = ["#2563eb", "#16a34a", "#dc2626"]
...
export function ComparadorClient({ allZones, initialSlugs }: ComparadorClientProps) {
  ...
  const fetchComparison = useCallback(async (slugs: string[]) => {
    ...
    setLoading(true)
    try {
      const data = await getZoneComparisonData(allZones, slugs)
      setCompData(data)
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Selecciona hasta 3 zonas para comparar sus métricas lado a lado.
          </p>
        </div>
        {selectedSlugs.length > 1 && (
          <button
            onClick={copyShareUrl}
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-full text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
          >
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 space-y-4 shadow-sm">
        ...
        {selectedSlugs.length === 0 && (
          <p className="text-sm text-slate-400 dark:text-slate-500">
            Ninguna zona seleccionada. Agrega hasta 3 zonas para comparar.
          </p>
        )}
      </div>
      ...
      {loading || isPending ? (
        ...
        <>
          ...
          <div className={`grid gap-4 ${selectedZones.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
      ...
      {selectedSlugs.length === 0 && (
        <div className="flex flex-col items-center gap-4 py-20 text-slate-400">
          <Icon name="compare" className="text-5xl" />
          <p className="text-sm font-medium">Selecciona al menos 2 zonas para comparar</p>
          <div className="flex flex-wrap gap-2 justify-center max-w-md">
            {allZones.slice(0, 4).map((z) => (
              <button
                key={z.zone_slug}
                onClick={() => addZone(z.zone_slug)}
                className="px-4 py-1.5 bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 rounded-full text-sm font-semibold hover:bg-blue-100 dark:hover:bg-blue-950/50 transition-colors"
              >
                {z.zone_name}
              </button>
            ))}
