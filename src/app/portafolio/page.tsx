...
import { PORTFOLIO_PRESETS, TIJUANA_ZONES, ZONE_RISK_DATA } from "@/lib/mock-data"
...
export default function PortafolioPage() {
  ...
  const preset = PORTFOLIO_PRESETS.find((p) => p.id === selectedPreset)!

  // Compute weighted metrics
  const weightedReturn = preset.allocations.reduce((sum, alloc) => {
    const zone = TIJUANA_ZONES.find((z) => z.zone_slug === alloc.zone_slug)
    return sum + (zone?.price_trend_pct ?? 0) * (alloc.allocation_pct / 100)
  }, 0)

  const weightedRisk = preset.allocations.reduce((sum, alloc) => {
    const risk = ZONE_RISK_DATA.find((r) => r.zone_slug === alloc.zone_slug)
