...
export function PricePercentile({ zonePrice, allZonePrices, zoneName }: PricePercentileProps) {
  ...
  const idx = sorted.findIndex((z) => z.name === zoneName)
  if (idx === -1 || sorted.length === 0) return null
  const rank = idx + 1
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl p-5 card-shadow border border-slate-100 dark:border-slate-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
        <div>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">Más barata</p>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg py-2">
          <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">Esta zona</p>
        ...
        <div>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">Más cara</p>
