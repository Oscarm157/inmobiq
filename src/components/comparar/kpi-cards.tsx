      ...
      {zones.map((zone, i) => {
        ...
        return (
          <div
            ...
            style={{ backgroundColor: `${zoneColor}08` }}
          >
            {/* Colored header */}
            <div
              className="px-6 pt-5 pb-4"
              style={{ background: `linear-gradient(135deg, ${zoneColor}12, ${zoneColor}06)` }}
            >
                    ...
                    {Object.entries(zone.listings_by_type)
                      ...
                      .map(([type, count]) => (
                        <span key={type} className="text-[9px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
                          ...
                          <span className="text-slate-400 dark:text-slate-500">{Math.round((count / typeTotal) * 100)}%</span>
                        </span>
                      ))}
                  </div>
                </div>
